...
type NextConfigFn = (phase: string, defaults: { defaultConfig: NextConfig }) => NextConfig | Promise<NextConfig>;
...
async function dotenvxNextConfigFn(
  ...
  if (typeof nextConfig === 'function') {
    resolvedNextConfig = { ...await (nextConfig as NextConfigFn)(phase, defaults) };
  } else {
    ...
    if (shouldTrace && resolvedFiles.length > 0) {
      // Shallow-copy experimental too before mutating
      resolvedNextConfig.experimental = { ...resolvedNextConfig.experimental };
      resolvedNextConfig.experimental.outputFileTracingIncludes ??= {};

      const tracing = resolvedNextConfig.experimental.outputFileTracingIncludes;
    ...
    if (isTurbopack) {
      ...
      try {
        ...
        console.warn('[dotenvx-next] Could not load init bundle for turbopack injection:', err);
      }
    }
    ...
    resolvedNextConfig.webpack = (webpackConfig: Record<string, unknown>, webpackOptions: Record<string, unknown>) => {
      // Apply user's existing webpack customizations first
      const config = prevWebpack
        ? (prevWebpack as (cfg: Record<string, unknown>, opts: Record<string, unknown>) => Record<string, unknown>)(webpackConfig, webpackOptions)
        : webpackConfig;
      ...
      const alias = (config.resolve as Record<string, unknown>).alias as Record<string, string>;
      ...
      try {
        ...
        debugLog('Could not resolve @fantasticfour/dotenvx-next/next-env for alias — skipping');
      }
      ...
      if (!isTurbopack) {
        try {
          const initSource = loadInitSource();
          const plugin = new DotenvxWebpackPlugin({ files: resolvedFiles, initSource });
          (config.plugins as unknown[]).push(plugin);
        } catch (err) {
          console.warn('[dotenvx-next] Could not load init bundle for webpack injection:', err);
        }
      }
